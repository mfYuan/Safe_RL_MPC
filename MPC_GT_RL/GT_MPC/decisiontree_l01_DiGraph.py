import environment_DiGraph

# robust decision tree
# level 0 driver assumes that all other cars are static

# Decision Tree action search
def decisiontree_l0(X_old, car_id, action_space, params, Level_ratio, L_matrix_all):
    ...
    if X_old1[4, car_id] == 1: # if the car is AV
        for id_1 in range(0, action_space.size): # for each action
            for dist_id_1 in range(0, len(dist_comb)): # for each disturbance combination
                ...
                X_new, R1 = environment_DiGraph.environment(X_old1, car_id, id_1, t_step_DT, params, dist_id_1, Level_ratio, L_matrix_all)
                ...
                for id_2 in range(0, action_space.size):
                    for dist_id_2 in range(0, len(dist_comb)):
                        k = 1
                        X_old1 = Buffer[k]
                        X_new, R2 = environment_DiGraph.environment(X_old1, car_id, id_2, t_step_DT, params, dist_id_2, Level_ratio, L_matrix_all)
                    ...
                    #Unceertainity in the worst case of second prediction step
                    Q_value_2_min[id_1][dist_id_1][id_2] = [min(Q_value_2[id_2][:]) + R1] 
            #Uncertainity in the worst case of first prediction step
        ...
        for id_1 in range(0, action_space.size):
            k = 0
            X_old1 = Buffer[k]
            X_new, R1 = environment_DiGraph.environment(X_old1, car_id, id_1, t_step_DT, params, dist_id, Level_ratio, L_matrix_all)
            ...
            for id_2 in range(0, action_space.size):
                k = 1
                X_old1 = Buffer[k]
                X_new, R2 = environment_DiGraph.environment(X_old1, car_id, id_2, t_step_DT, params, dist_id, Level_ratio, L_matrix_all)
