...
def plot_level_ratio(Level_ratio_history, ego_car_id, opp_car_id, params, step, episode, max_step, fig, time_step):
    ...
    condition1 = np.arange(0, (step+1)*params.t_step_DT, params.t_step_DT).shape
    condition2 = np.squeeze(Level_ratio_history[episode, 0:step+1, ego_car_id * (num_cars - 1) + opp_car_id - 1, 0]).shape
