...
def reward_sim(X_reward, car_id, action_id, params):
    ...
    if X_reward[8,car_id] < X_reward[6,car_id]:
        Complete = Complete + 1e-5 * Complete_Penalty*abs(X_reward[8,car_id] - X_reward[6,car_id])/abs(X_reward[6,car_id]-X_reward[5,car_id])
    else:
        complete_flag[episode,car_id] = 1
 
    # Speed reward
    Speed = 0*(X_reward[3, car_id])
    ...
    # Local Reward
    R_l = ( Colli  + Safe  + Complete*0 + Speed)
