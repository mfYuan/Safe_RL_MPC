...
def plot_sim(X_old, params, step, Level_ratio, fig, counter, path):
    ...
    for id in range(0,len(X_old[0,:])):
        ...
        if id == ego_car_id:
            ...
            plt.plot(wps_tp[:, 0], wps_tp[:, 1], color=color[-1], LineWidth=car_rect_lw+1, linestyle=':')
            
            wps_0 = np.array(Heading_traj_0[int(X_old[8, id]):int(X_old[8, id])+W_curr_ls[0]])               
            plt.plot(wps_0[:, 0], wps_0[:, 1], color=color[0], LineWidth=car_rect_lw+1)
            
            wps_2 = np.array(Heading_traj[int(X_old[8, id]):int(X_old[8, id])+W_curr_ls[1]])
            plt.plot(wps_2[:, 0], wps_2[:, 1], color=color[2], LineWidth=car_rect_lw+1)
            ...
            plt.plot(wps_3[:, 0], wps_3[:, 1], color=color[3], LineWidth=car_rect_lw+1)
            ...
            try:
                ...
                plt.plot(wps_4[:, 0], wps_4[:, 1], color=color[4], LineWidth=car_rect_lw+1)
            except:
                pass  
    ...
    if display:
        ...
        for id in range(0, len(X_old[0,:])):
            if id ==0:
                ax.annotate('car 1', xy=(X_old[0, id], X_old[1, id]-move), size=label_font_size)
            elif id ==1:
                ax.annotate('Ego', xy=(X_old[0, id], X_old[1, id]-move) ,size=label_font_size)
            elif id ==2:
                ax.annotate('car 2', xy=(X_old[0, id], X_old[1, id]-move) ,size=label_font_size)
            elif id ==3:
                ax.annotate('car 3', xy=(X_old[0, id], X_old[1, id]-move) ,size=label_font_size)
            elif id ==4:
                ax.annotate('car 4', xy=(X_old[0, id], X_old[1, id]-move) ,size=label_font_size)
    ...
    plt.savefig(path[0]+'/'+str(counter)+'.png', dpi = 100)
