...
def reward(X_reward, car_id, action_id, params, dist_id, Level_ratio):
    ...
    Ego_rectangle_safe = Polygon(
            [[X_reward[0,car_id]-l_car/2*math.cos(X_reward[2,car_id])+w_car_safe/2*math.sin(X_reward[2,car_id]),
            X_reward[1,car_id]-l_car/2*math.sin(X_reward[2,car_id])-w_car_safe/2*math.cos(X_reward[2,car_id])],
            [X_reward[0,car_id]-l_car/2*math.cos(X_reward[2,car_id])-w_car_safe/2*math.sin(X_reward[2,car_id]),
            X_reward[1,car_id]-l_car/2*math.sin(X_reward[2,car_id])+w_car_safe/2*math.cos(X_reward[2,car_id])],
    ...
    if X_reward[8,car_id] < X_reward[6,car_id]:
        Complete = Complete + 1e4 * Complete_Penalty*abs(X_reward[8,car_id] - X_reward[6,car_id])/abs(X_reward[6,car_id]-X_reward[5,car_id])
    else:
