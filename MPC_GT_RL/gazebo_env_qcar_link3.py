...
class envmodel3():
    def __init__(self):
        ...
        self.rearPose = rospy.Subscriber('/' + self.agentrobot3 + '/rear_pose', PoseStamped, self.pose_cb, queue_size=1)
    ...
    def SteeringCommand(self, X_old, car_id, params):
        ...
        x = self.robotstate3[0]#X_old[0][car_id]
        y = self.robotstate3[1]#X_old[1][car_id]
        yaw = self.robotstate3[4]#X_old[2][car_id]
        v = self.robotstate3[2]#X_old[3][car_id]
    ...
    def speed(self, pos, action, params):
        ...
        if delta_t > 1.0:
            delta_t = 0.1
        speed = self.robotstate3[2] + self.qcar_dict[action]*delta_t
