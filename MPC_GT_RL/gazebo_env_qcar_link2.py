...
class envmodel2():
    def __init__(self):
        ...
        #     '/' + self.agentrobot2 + '/csi_front/image_raw', Image, self.image_callback)
        # self.subLaser = rospy.Subscriber(
        #     '/' + self.agentrobot2 + '/lidar', LaserScan, self.laser_states_callback)
    ...
    def distance_callback(self, data):
        ...
        self.d_2_3 = data.pose.position.z
    ...
    def getreward(self):
        ...
        if self.other_side == True:
            reward = reward + 2
            #print("[Link2] Reached other side! Gain 2 reward.")

        # Collision (-1 reward)
        if self.d_1_2 < 0.2:
            reward = reward - 5
            print("[Link2] Vehicle Collision: Lose 5 reward.")
    ...
    def get_env(self):
        ...
        # robotstate--->x,y,v,w,yaw,vx,vy
        selfstate[0] = self.robotstate2[2]  # v
        selfstate[1] = '2'  # v

        if self.qcar1_path == '10':
            selfstate[2] = self.qcar4_p2
            selfstate[3] = 0.0
            selfstate[4] = self.qcar1_p2
            selfstate[5] = self.qcar2_p1
            selfstate[6] = 0.0
            selfstate[7] = self.qcar2_p1
        elif self.qcar1_path == '5':
            selfstate[2] = self.qcar4_p2
            selfstate[3] = 0.0
            selfstate[4] = self.qcar1_p4
            selfstate[5] = self.qcar2_p1
            selfstate[6] = 0.0
            selfstate[7] = self.qcar2_p2
        elif self.qcar1_path == '4':
            selfstate[2] = self.qcar4_p2
            selfstate[3] = 0.0
            selfstate[4] = self.qcar1_p5
            selfstate[5] = self.qcar2_p1
            selfstate[6] = 0.0
            selfstate[7] = self.qcar2_p3
        ...
        # selfstate[1] = self.robotstate2[3]  # w
        ...
        # # ----------------------------------------
        # dx = -(self.robotstate2[0]-self.robotstate1[0])
        # dy = -(self.robotstate2[1]-self.robotstate1[1])
        # xp = dx*math.cos(self.robotstate2[4]) + \
        #     dy*math.sin(self.robotstate2[4])
        # yp = -dx*math.sin(self.robotstate2[4]) + \
        #     dy*math.cos(self.robotstate2[4])
        ...
        # 是否到达目标点判断
        # If qcar2 reaches end of path 5 (0.18, 0.94)
        if (self.robotstate2[0] > 0.18 and self.robotstate2[1] > 0.94 and self.path == '5'):
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")

        # If qcar2 reaches end of path 6 (-0.94, 0.18)
        elif (self.robotstate2[0] < -0.94 and self.path == '6'):
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")

        # If qcar2 reaches end of path 7 (-0.18, -0.94)
        elif (self.robotstate2[0] < -0.18 and self.robotstate2[1] < -0.94 and self.path == '7'):
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")

        # If qcar2 reaches end of path 8 (0.94, -0.18)
        elif (self.robotstate2[0] > 0.94 and self.path == '8'):
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")
    ...
    def SteeringCommand(self, X_old, car_id, params):
        ...
        x = self.robotstate2[0]#X_old[0][car_id]
        y = self.robotstate2[1]#X_old[1][car_id]
        yaw = self.robotstate2[4]#X_old[2][car_id]
        v = self.robotstate2[2]#X_old[3][car_id]
    ...
    def speed(self, pos, action, params):
        ...
        if delta_t > 1.0:
            delta_t = 0.1
        speed = self.robotstate2[2] + self.qcar_dict[action]*delta_t
        self.previous = t1
        if speed >0.6:
            speed = 0.6
        elif speed < 0:
