...
class envmodel4():
    def __init__(self):
        ...
        #     '/' + self.agentrobot4 + '/lidar', LaserScan, self.laser_states_callback)

        self.rearPose = rospy.Subscriber('/' + self.agentrobot4 + '/rear_pose', PoseStamped, self.pose_cb, queue_size=1)
    ...
    def get_env(self):
        ...
        if (self.robotstate4[0] > 1.07 and self.robotstate4[1] < -0.18 and self.path == '12'):
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")

        # If qcar4 reaches end of path 7 (-0.18, -0.94)
        elif (self.robotstate4[1] < -0.94 and self.path == '7'):
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")

        # If qcar4 reaches end of path 2 (-0.94, 0.18)
        elif (self.robotstate4[0] < -0.94 and self.robotstate4[1] > 0.18 and self.path == '2'):
            self.other_side = True
            self.done_list = True  # 终止 (terminated due to episode completed)
            #print("Reached other side!")
    ...
    def SteeringCommand(self, X_old, car_id, params):
        ...
        x = self.robotstate4[0]#X_old[0][car_id]
        y = self.robotstate4[1]#X_old[1][car_id]
        yaw = self.robotstate4[4]#X_old[2][car_id]
        v = self.robotstate4[2]#X_old[3][car_id]
    ...
    def speed(self, pos, action, params):
        ...
        if delta_t > 1.0:
            delta_t = 0.1
        speed = self.robotstate4[2] + self.qcar_dict[action]*delta_t
        self.previous = t1
        if speed >0.6:
            speed = 0.6
        elif speed < 0:
