...
from gazebo_env_qcar_link3 import envmodel3
...
#     tf.Session(graph=graph3, config=config3))
...
class DQN3:
    ...
    def initialize_network(self):
        ...
        self.sess3, self.saver = self.init_sess()
    ...
    def init_sess(self):
        with graph3.as_default():
            ...
            #sess3 = tf.InteractiveSession(config=config3)
            sess3 = tf.Session(graph=graph3, config=config3)

            self.save_location = 'saved_networks/' + 'two_qcar_links_' + \
                self.date_time + '_' + self.Number + '/qcar3'
            ...
            check_save = input('Load Model for Link 3? (1=yes/2=no): ')

            if check_save == 1:
                # Restore variables from disk.
                saver.restore(sess3, self.load_path1 + "/model.ckpt")
                print("Link 3 model restored.")
        ...
        with graph3.as_default():
            if progress == "Observing":
                ...
                if random.random() < Epsilon:
                    ...
                    #                            observation_stack], self.x_sensor: [state_stack]}, session=self.sess3)
                    Q_value = self.output.eval(
                        feed_dict={self.x_sensor: [state_stack]}, session=self.sess3)
                    action = np.zeros([self.Num_action])
    ...
    def train(self, minibatch, w_batch, batch_index):
        with graph3.as_default():
            ...
            #                              self.x_image: observation_next_batch, self.x_sensor: state_next_batch}, session=self.sess3)
            Q_network = self.output.eval(
                feed_dict={self.x_sensor: state_next_batch}, session=self.sess3)
            ...
            _, self.loss, TD_error_batch = self.sess3.run([self.train_step, self.loss_train, self.TD_error], feed_dict={self.action_target: action_batch,
    ...
    def Experience_Replay(self, state, action, reward, next_state, terminal):
        with graph3.as_default():
            ...
            if self.progress == 'Observing':
                ...
                #                                   next_observation], self.x_sensor: [next_state]}, session=self.sess3)
                Q_batch = self.output_target.eval(
                    feed_dict={self.x_sensor: [next_state]}, session=self.sess3)
                if terminal == True:
                    y = [reward]
                else:
                ...
                #                               observation], self.x_sensor: [state]}, session=self.sess3)[0]
                TD_error = self.TD_error.eval(feed_dict={self.action_target: [
                                              action], self.y_target: y, self.x_sensor: [state]}, session=self.sess3)[0]
                self.TD_list = np.append(self.TD_list, pow(
    ...
    def save_model(self):
        with graph3.as_default():
            ...
            self.save_path = self.saver.save(
                self.sess3, self.save_location + "/" + number + "/model.ckpt")
            # ------------------------------
    ...
    def return_action(self):
        ...
        #     self.progress, self.sess3, self.observation_stack, self.state_stack, self.Epsilon)
        self.action, Q_value = self.select_action(
            self.progress, self.sess3, self.state_stack, self.Epsilon)
        # print(Q_value)
    ...
    def print_information(self):
        print('[Link3-'+self.progress+'] step:'+str(self.step)+'/episode:'+str(self.episode) +
    ...
    def new_environment(self):
        ...
        if self.progress != 'Observing':
            ...
            # # ------------------------------
            # np.savetxt(self.save_location + '/qcar3_reward.txt',
            ...
            #     np.savetxt(self.save_location + '/qcar3_25avg_score.txt',
    ...
    def save_fig(self):
        plt.savefig(self.save_location + '/qcar3_reward.png')
        plt.show()
    ...
    def update_model(self, level):
        with graph3.as_default():
            ...
            # Load level 1 model
            if self.level == "1":
                self.saver.restore(self.sess3, self.load_path1 + "/model.ckpt")
            # Load level 2 model
            elif self.level == "2":
                self.saver.restore(self.sess3, self.load_path2 + "/model.ckpt")
            else:
                self.level = "0"
                print("Error: Model restore failed.")

    def set_velocity(self, v1, v2, v4, s1, s2, s4):
        self.env.set_velocity(v1, v2, v4, s1, s2, s4)
