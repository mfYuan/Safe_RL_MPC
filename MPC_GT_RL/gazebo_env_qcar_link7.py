...
class envmodel7():
    def __init__(self):
        ...
        self.rearPose = rospy.Subscriber('/' + self.agentrobot7 + '/rear_pose', PoseStamped, self.pose_cb, queue_size=1)
    ...
    def SteeringCommand(self, X_old, car_id, params):
        ...
        x = self.robotstate7[0]#X_old[0][car_id]
        y = self.robotstate7[1]#X_old[1][car_id]
        yaw = self.robotstate7[4]#X_old[2][car_id]
        v = self.robotstate7[2]#X_old[3][car_id]
    ...
    def speed(self, pos, action, params):
        ...
        if delta_t > 1.0:
            delta_t = 0.1
        speed = self.robotstate7[2] + self.qcar_dict[action]*delta_t
        self.previous = t1
        if speed >0.6:
            speed = 0.6
        elif speed < 0:
